// worker-v1011.js - Final Attempt with Absolute Paths
let isReady = false;
let recognizer = null;
let stream = null;
...
async function getPersistentFile(url, name) {
    try {
        ...
        if (file.size > 0) {
            logToMain('status', `HIT: ${name} (${(file.size/1024/1024).toFixed(1)}MB)`);
            return await file.arrayBuffer();
        }

        logToMain('status', `FETCH: ${name}...`);
        ...
        logToMain('status', `SAVED: ${name}`);
        return buffer;
    } catch (e) {
        logToMain('error', `OPFS Error: ${e.message}`);
        const res = await fetch(url);
...
async function init() {
    try {
        logToMain('status', 'Booting V1011...');
        
        // GH Pages absolute paths
        const root = '/lilith-stt-v3/src/lib/';
        const wasmBinary = await getPersistentFile(root + 'sherpa-onnx-asr.wasm', 'sherpa-onnx-asr.wasm');
        ...
        return new Promise((resolve, reject) => {
            self.Module = {
                ...
                onRuntimeInitialized: async () => {
                    try {
                        logToMain('status', 'Runtime Ready. Loading Models...');
                        ...
                        logToMain('status', 'Constructing Recognizer...');
                        ...
                        logToMain('error', `Sub-init error: ${e.message}`);
                        reject(e);
                    }
                }
            };

            importScripts(root + 'sherpa-onnx-asr.js');
        });
    } catch (err) {
        logToMain('error', `Global Error: ${err.message}`);
    }
}
...
self.onmessage = async (e) => {
    if (e.data.type === 'init') await init();
        ...
        if (result.text) {
            self.postMessage({ type: 'result', data: { text: result.text } });
        }
    }
};
